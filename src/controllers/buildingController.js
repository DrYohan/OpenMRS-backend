...
const BuildingController = {
  ...
  async updateBuilding(req, res) {
    try {
      ...
      const filesToDelete = currentImages.filter(
        (f) => !existingFiles.includes(f)
      );
      filesToDelete.forEach((filePath) => {
        const fullPath = path.join(
          __dirname,
          "../uploads/building_uploads",
      ...
      res
        .status(200)
        .json({ message: "Building record updated successfully!" });
    } catch (error) {
  ...
  async getBuildingIdByLandId(req, res) {
    try {
      ...
      if (!landId) {
        return res
          .status(400)
          .json({ success: false, message: "landId is required" });
      }
