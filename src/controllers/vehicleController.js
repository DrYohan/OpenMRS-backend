...
const VehicleController = {
  async createVehicle(req, res) {
    try {
      ...
      const regExists = await VehicleService.checkRegistrationIdExist(
        data.registrationId
      );
      if (regExists) {
        return res.status(400).json({
          error: `Registration ID '${data.registrationId}' already exists`,
        });
      }

      const chasisExists = await VehicleService.checkChasisNoExist(
        data.chasisNo
      );
      if (chasisExists) {
        return res
          .status(400)
          .json({ error: `Chasis No '${data.chasisNo}' already exists` });
      }

      const engineExists = await VehicleService.checkEngineNoExist(
        data.engineNo
      );
      if (engineExists) {
        return res
          .status(400)
          .json({ error: `Engine No '${data.engineNo}' already exists` });
      }
      ...
      if (error.code === "ER_DUP_ENTRY") {
        return res
          .status(400)
          .json({ error: "Duplicate registration, chasis, or engine number" });
      }
  ...
  async fetchPendingApprovalVehicles(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Data fetched successfully", data: rows || [] });
    } catch (error) {
  ...
  async getAllSuppliers(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Suppliers fetched successfully", data: rows || [] });
    } catch (error) {
  ...
  async updateVehicle(req, res) {
    try {
      ...
      if (
        await VehicleService.checkRegistrationIdExistForUpdate(
          data.registrationId,
          vehicleId
        )
      ) {
      ...
      if (
        await VehicleService.checkChasisNoExistForUpdate(
          data.chasisNo,
          vehicleId
        )
      ) {
      ...
      if (
        await VehicleService.checkEngineNoExistForUpdate(
          data.engineNo,
          vehicleId
        )
      ) {
      ...
      const [rows] = await pool.execute(
        "SELECT file FROM vehicles WHERE id = ?",
        [vehicleId]
      );
      if (!rows.length)
        return res.status(404).json({ error: "Vehicle not found" });
      ...
      const frontendExistingFiles = data.existingFiles
        ? Array.isArray(data.existingFiles)
          ? data.existingFiles
          : [data.existingFiles]
        : [];

      const filesToDelete = existingFilePaths.filter(
        (f) => !frontendExistingFiles.includes(f)
      );
      ...
      const finalFilePaths =
        [...frontendExistingFiles, ...newFiles].join("@@@") || null;
...
`,
      ...
      console.error(
        "Error fetching sub categories by middle categories:",
        error
      );
      return res.status(500).json({ error: "Internal server error" });
    }
  ...
  async getAllVehicles(req, res) {
    try {
      ...
      const [[countResult]] = await pool.query(
        "SELECT COUNT(*) AS total FROM vehicles_master"
      );

      // 🚨 LIMIT & OFFSET MUST BE DIRECT VALUES
      const sql = `
      SELECT *
      FROM vehicles_master
    ...
    `;
      ...
      console.error("Error fetching vehicles master:", error);
