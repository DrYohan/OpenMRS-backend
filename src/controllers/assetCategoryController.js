...
const assetCategoryController = {
  ...
  async getAllSubCategories(req, res) {
    try {
      ...
      const categories = await AssetCategory.getAllSubCategories(filters);
      res.json({
        success: true,
        count: categories.length,
        data: categories,
  ...
  async createSubCategory(req, res) {
    try {
      ...
      // Check if sub_category_id already exists
      const existingById = await AssetCategory.getSubCategoryByCode(
        sub_category_id
      );
      if (existingById) {
        return res
          .status(400)
          .json({ error: "Sub Category ID already exists" });
      }

      // Check if short_code already exists
      const [existingByCode] = await pool.execute(
        "SELECT * FROM fixed_asset_sub_categories WHERE short_code = ?",
        [short_code]
      );
      ...
      const subCategoryData = {
        ...
        sub_category_name,
        short_code,
        description_method: description_method || null,
        percentage: percentage || null,
        item_locator_color: item_locator_color || null,
        description: description || null,
      };

      const result = await AssetCategory.createSubCategory(subCategoryData);
  ...
  async getSubCategoryById(req, res) {
    try {
      const { id } = req.params;
      const category = await AssetCategory.getSubCategoryById(id);
  ...
  async updateSubCategory(req, res) {
    try {
      ...
      const existing = await AssetCategory.getSubCategoryById(id);
      if (!existing) {
        return res.status(404).json({ error: "Sub category not found" });
      }

      // If updating short_code, check for uniqueness
      if (
        updateData.short_code &&
        updateData.short_code !== existing.short_code
      ) {
        const [existingCode] = await pool.execute(
          "SELECT * FROM fixed_asset_sub_categories WHERE short_code = ? AND id != ?",
          [updateData.short_code, id]
        );
      ...
      const updated = await AssetCategory.updateSubCategory(id, updateData);
  ...
  async deleteSubCategory(req, res) {
    try {
      ...
      const deleted = await AssetCategory.deleteSubCategory(id);
  ...
  async getSubCategoriesByMiddleCategory(req, res) {
    try {
     ...
     console.log("comming getSubCategoriesByMiddleCategory", middle_category_id)
      ...
      console.error("Error fetching sub categories by middle category:", error);
      res.status(500).json({ error: "Internal server error" });
    }
  },
